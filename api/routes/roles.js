    ...
    try {
        ...
        if(!body.permissions || !Array.isArray(body?.permissions) || body?.permissions?.length == 0){
            throw new CustomError(
                Enum.HTTP_CODES.BAD_REQUEST, i18n.translate("COMMON.VALIDATION_ERROR_TITLE", req.user?.language), i18n.translate("COMMON.FIELD_MUST_BE_TYPE", req.user?.language, ["permission, Array"]));
        }
...
router.post("/delete", auth.checkRoles("role_delete"), async(req, res) => {
    ...
    try {
        ...
        if(!body._id) throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST, i18n.translate("COMMON.VALIDATION_ERROR_TITLE", req.user?.language), i18n.translate("COMMON.FIELD_MUST_BE_FILLED", req.user?.language, ["_id"]));
        await Roles.roluSilAyniZamanadaBagliRolePriviligesideSil({_id: body._id});
